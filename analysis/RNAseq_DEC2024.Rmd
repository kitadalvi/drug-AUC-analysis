...
sampleinfo <- read.delim2('/Volumes/bioinf/home/ndalvi/RNAseq_STAR/Results/HTseq/sampleinfo_original.txt', header=TRUE, check.names = FALSE, stringsAsFactors = TRUE)
```
...
boxplot(lcpm, las=1, col=col, main="A. Example: Unnormalised data", outline=FALSE)
...
legend("topleft",legend=levels(sampleinfo$Type), fill = c('#D95F02','#1B9E77','#7570B3','#E7298A',"#66A61E"))
title('A. Sample Type')
...
type_annotation <- data.frame(Type=sampleinfo[,c(2)],Batch=sampleinfo[,c(5)])
...
order<- DEgenes$genes[1:100]
top_100 <- lcpm[order[1:100],]
pattern_annotation <- data.frame(Pattern=sampleinfo[,c(4)],Batch=sampleinfo[,c(5)])
