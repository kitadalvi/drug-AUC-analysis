...
sampleinfo <- read.delim2('/Volumes/bioinf/home/ndalvi/RNAseq_STAR/Results/HTseq/sampleinfo.txt', header=TRUE, check.names = FALSE, stringsAsFactors = TRUE)
```
...
y<- DGEList(counts=rawdata[,2:58], genes=rawdata[,1])
...
design <- model.matrix(~pattern+type+batch)
...
y <- y[,-c(50,39,56,36)]
sampleinfo <- sampleinfo[-c(50,39,56,36),]
rownames(sampleinfo)<- sampleinfo$`Sample-ID`
design <- design[-c(50,39,56,36),]
```
...
col.pattern <- brewer.pal(3, 'Dark2')[sampleinfo$Pattern]
...
legend("topleft",legend=levels(sampleinfo$Pattern), fill = c('#7570B3','#D95F02','#1B9E77'))
...
• 656 DE genes were upregulated

• 3928 DE genes were downregulated


#### Visualisation

##### Basic Heatmap
After ordering our table of DE genes (both upregulated and downregulated) by p-value, we cans elect the top 100 most significantly differentially expressed genes between ORG and TUM samples:
...
type_annotation <- data.frame(Type=sampleinfo[,c(2)],
                              Batch=sampleinfo[,c(5)], 
                              TUM_type = sampleinfo[,c(7)],
                              Stage = sampleinfo[,c(8)],
                              Age = sampleinfo[,c(9)])
rownames(type_annotation) <- sampleinfo$`Sample-ID`
pheatmap(top_100,
         border_color = 'NA',
         cellwidth = 15,
         cellheight = 10, 
         ...
         main="Top 100 DE genes between ORG and TUM samples",
         fontsize = 15)
```
...
•  582 DE genes were upregulated

•  329 DE genes were downregulated
...
order<- DEgenes$genes[1:100]
top_100 <- lcpm[order[1:100],]
pattern_annotation <- data.frame(Pattern=sampleinfo[,c(4)],
                                 Batch=sampleinfo[,c(5)],
                                 TUM_type = sampleinfo[,c(7)],
                                 Stage = sampleinfo[,c(8)],
                                 Age = sampleinfo[,c(9)])
rownames(pattern_annotation) <- sampleinfo$`Sample-ID`
pheatmap(top_100,
         border_color = 'NA',
         cellwidth = 15,
         cellheight = 10, 
...
```{r}
### Subset DGE object and sample info to only include relevant ORGs
y2 <- y[,c("ORG-38-ORG","ORG41","ORG-46-ORG","ORG55","ORG60","ORG64_new","ORG65","ORG66","ORG70","ORG73")]
sampleinfo2 <- sampleinfo
rownames(sampleinfo2)<- sampleinfo2$`Sample-ID`
sampleinfo2 <- sampleinfo2[c("ORG-38-ORG","ORG41","ORG-46-ORG","ORG55","ORG60","ORG64_new","ORG65","ORG66","ORG70","ORG73"),]

### Create new design matrix
batch <- as.factor(c(sampleinfo2$Batch))
pattern <- as.factor(c(sampleinfo2$Pattern))
responder <- as.factor(c(sampleinfo2$Responder))
design <- model.matrix(~batch+responder)
rownames(design)<- rownames(sampleinfo2)

### Making contrasts
fit <- glmQLFit(y2, design, robust=TRUE)
res <- glmQLFTest(fit, coef=c("responderSENS"))
res <- res[res$table$logFC<= -2 | res$table$logFC>=2,]
DEgenes <- topTags(res, p.value=0.05, n=100000)
DEgenes <- DEgenes$table[DEgenes$table$logFC<= -2 | DEgenes$table$logFC>=2,]
```
...
•   281 DE genes were upregulated

•   52 DE genes were downregulated
...
```{r, fig.height=18, fig.width=18}
order<- DEgenes$genes[1:100]
lcpm <- cpm(y2, prior.count=2, log=TRUE)
rownames(lcpm)<- y2$genes$genes
top_100 <- lcpm[order[1:100],]
pattern_annotation <- data.frame(Pattern=sampleinfo[,c(4)],
                                 Batch=sampleinfo[,c(5)],
                                 TUM_type = sampleinfo[,c(7)],
